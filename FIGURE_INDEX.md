...
## 1. Two-Axis Gimbal Motion

![Two-axis TVC gimbal motion](media/tvc_gimbal_motion.svg)

[Browser-viewable motion animation](media/tvc_gimbal_animation.html)

**What it shows:** The intended kinematics of the Rev A TVC stand: yaw rotates the outer frame, pitch rotates the inner carrier, and the nozzle/thrust vector follows the combined motion.

**Upper-division physical interpretation:** The animation distinguishes kinematics from dynamics. Kinematically, pitch and yaw are commanded rotations. Dynamically, the axes are not equivalent: yaw carries the pitch subsystem, so `I_yaw` is larger than `I_pitch`. For the same useful servo torque, yaw should have lower angular acceleration, longer settling time, and greater sensitivity to backlash, frame compliance, and servo phase lag.

## 2. Rev A CAD Baseline
...
## 4. Pitch Pre-Hardware Step Prediction
...
## 5. Yaw Pre-Hardware Step Prediction
...
## 6. Pre-Hardware Model Parameters
...
## 7. Calibration And First-Test Worksheet
